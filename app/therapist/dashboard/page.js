...
function ReportSection({ title, reports, borderColor, urgentBg, badgeColor, uploading, setUploading, onUploaded }) {
  if (reports.length === 0) return null
  return (
    <div style={{ background: urgentBg, border: `1px solid ${borderColor}`, borderRadius: '12px', padding: '1rem 1.25rem', marginBottom: '1rem' }}>
      <div style={{ fontWeight: '600', color: badgeColor, marginBottom: '8px', fontSize: '14px' }}>
        {title} — {reports.length} pending
      </div>
        ...
        {reports.map((r, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              border: `1px solid ${urgent ? '#F09595' : r.file_url ? '#97C459' : borderColor}`,
              ...
              <div style={{ flex: 1 }}>
                ...
                {r.file_url && (
                  <div style={{ fontSize: '11px', color: '#27500A', marginTop: '4px' }}>✓ Uploaded {r.uploaded_at}</div>
                )}
              </div>
    ...
    <div style={{ minHeight: '100vh', background: '#f8f9fa', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '1.5rem' }}>
        ...
        <ReportSection
          ...
          reports={pendingReports.filter(r => r.doc_type !== 'IE Report')}
          borderColor="#EF9F27"
          urgentBg="#FAEEDA"
          badgeColor="#633806"
          uploading={uploading}
